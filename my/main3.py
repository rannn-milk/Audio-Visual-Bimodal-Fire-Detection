...
# ============================ 训练函数 ============================
def run_one_attn(attn_method, train_data, test_data, device, config):
    T = config['T']
    ...
    print(f"\n==================================================")
    print(f"    开始训练 → 注意力方法: {attn_method}")
    print(f"==================================================")
    ...
    for epoch in range(epochs):
        ...
        scheduler.step()
        print(f"[{attn_method}] Epoch {epoch + 1:2d} | Loss: {losses_m.avg:.4f} | Acc@1: {top1_m.avg:.2f}%")
...
if __name__ == "__main__":
    ...
    # 遍历所有方法
    results = []
    for method in snn_methods:
        acc = run_one_attn(method, train_data, test_data, device, config)
        results.append((method, acc))

    # 输出最终表格
    print("\n" + "=" * 60)
    print("所有注意力方法训练完成！最终结果：")
    print("=" * 60)
    for method, acc in results:
        print(f"{method:<20} | 测试准确率 = {acc:.2f}%")